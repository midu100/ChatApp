import { useGetConversationQuery, useGetProfileQuery } from "../lib/api";
 ...
 */
const ConversationList = ({ profileData }) => {
  console.log("convoList=", profileData);
    ...
    <div className="w-[340px] bg-surface-panel flex flex-col border-r border-border-subtle h-full">
      ...
      <div className="px-4 py-3 bg-surface-hover flex items-center justify-between border-b border-border-subtle shrink-0">
        <div className="flex items-center gap-3">
          <Avatar
            name={profileData?.user?.fullName || "Midu"}
            image={profileData?.user?.avatar}
            size="sm"
            showStatus={true}
            isOnline={true}
          />
          <div>
            ...
            <p className="text-[11px] text-primary font-semibold">
              {profileData?.user?.email || "My Profile"}
            </p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar mt-1">
        {data?.map((item,i) => (
          <div key={i} className="w-full flex items-center gap-3 px-3 py-3 bg-surface-active cursor-pointer">
            ...
            <div className="flex-1 min-w-0 border-b border-border-subtle pb-3">
              <div className="flex items-center justify-between mb-0.5">
                <h3 className="text-[15px] font-medium text-text-primary truncate">
                  {
                    item?.creator?._id == profileData?.user?._id 
                    ?
